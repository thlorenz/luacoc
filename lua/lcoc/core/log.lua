...
function Log:set_level(level)
  -- package.loaded["lcoc.core.log"] = nil
  local log_level = Log.levels[level:upper()]
  local status_ok, logger = pcall(require("structlog").get_logger, "lcoc")
  if status_ok then
  ...
  require("packer.log").new { level = lcoc.log.level }
end
...
function Log:init()
  ...
  local log_level = Log.levels[(lcoc.log.level):upper() or "WARN"]
  local lcoc_log = {
    lcoc = {
      sinks = {
  ...
  structlog.configure(lcoc_log)
  local logger = structlog.get_logger "lcoc"

  -- Overwrite `vim.notify` to use the logger
  if lcoc.log.override_notify then
...
function Log:get_path()
  return string.format("%s/%s.log", get_cache_dir(), "lcoc")
end
